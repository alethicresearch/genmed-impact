...
The two legs of that argument — causal inference and correction — are advancing at very different rates, and recent work makes the asymmetry concrete. **AlphaGenome Atlas** precomputes predicted regulatory effects for all ~9 billion possible human single-nucleotide variants plus >100M observed biobank indels, and derives a unified variant-impact score from them (AlphaGenome Atlas team, Google DeepMind, 2026; resource at <https://alphagenome.google/atlas>). Its demonstrated clinical value is **diagnostic**: it raised recall of known pathogenic variants in solved rare-disease cases from 12.5% to 29.5% within each patient's top 50 ranked variants against a standard comparator, and helped resolve a previously unsolved epileptic encephalopathy case by identifying a deep intronic variant creating a brain-specific cryptic splice site.

That is an advance in *finding and interpreting* causal variants, and it belongs to the existing genetic-medicine stack rather than to editing — a variant nobody can classify cannot be screened for or selected against either. It moves none of the four gates on the editing ladder: not whether selection fails, not whether a correction route exists for the coding and structural lesions that dominate the residual, and least of all whether editing works in an embryo or is safe enough. The same work also records a constraint that bears directly on the polygenic frontier: the underlying model captures cis-regulatory grammar and does not model trans-acting mechanisms, which mediate a substantial proportion of complex-trait heritability.
