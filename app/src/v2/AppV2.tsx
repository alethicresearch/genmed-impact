...
export default function AppV2() {
  ...
  useEffect(() => {
    document.title = 'Reframing Genetic Editing in Terms of Medical Impact — v2';
    loadAll()
      .then(setData)
      ...
      <footer className="border-t border-slate-200 bg-slate-50">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-5 py-8 text-xs leading-5 text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-4">
            ...
            <a className="hover:text-blue-700" href={`${legacyBase}?tab=methods`}>
              Full methods
            </a>
            <a className="hover:text-blue-700" href={legacyBase}>
              v1 research page
            </a>
          </div>
        </div>
    ...
    <header className="border-b border-slate-200 bg-[linear-gradient(180deg,#f8fbff_0%,#ffffff_72%)]">
      <div className="mx-auto max-w-6xl px-5 pb-12 pt-8 sm:pt-12">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-4 text-sm">
          ...
          <a href={`${legacyBase}?tab=overview`} className="text-slate-500 hover:text-slate-800">
  ...
  return (
    <StorySection id="burden" number="01" eyebrow="Disease burden" title="How large is the modeled burden of serious genetic disease?">
      ...
      <ReaderTools mode={mode}>
        ...
        <ToolLink href={`${legacyBase}?tab=denominator`} label="Explore the burden model ↗" />
      </ReaderTools>
    </StorySection>
  );
}
  ...
  return (
    <StorySection id="impact-now" number="02" eyebrow="Impact now" title="How much can established genetic medicine already achieve?" tint>
      ...
      <ReaderTools mode={mode}>
        ...
        <ToolLink href={`${legacyBase}?tab=prevention`} label="Explore existing medicine ↗" />
      </ReaderTools>
    </StorySection>
  );
}
  ...
  return (
    ...
    >
      ...
      <ReaderTools mode={mode}>
        ...
        <ToolLink href={`${legacyBase}?tab=residual`} label="Explore the editing residual ↗" />
      </ReaderTools>
    </StorySection>
  );
}
  ...
  return (
    ...
    >
      ...
      <ReaderTools mode={mode}>
        ...
        <ToolLink href={`${legacyBase}?tab=embryos`} label="Explore selection versus correction ↗" />
      </ReaderTools>
    </StorySection>
  );
}

function FutureSection({ data, mode, legacyBase }: { data: AllData; mode: Mode; legacyBase: string }) {
  ...
  return (
    <StorySection id="future-impact" number="05" eyebrow="Future impact" title="What happens if the technological frontier moves?">
      ...
      <ReaderTools mode={mode}>
        ...
        <ToolLink href={`${legacyBase}?tab=multifactorial`} label="Explore the polygenic frontier ↗" />
      </ReaderTools>
    </StorySection>
  );
}

function PolicySection({ legacyBase, mode }: { legacyBase: string; mode: Mode }) {
  return (
    <StorySection id="policy" number="06" eyebrow="Ethics & policy" title="What follows from an impact-based framework?" tint>
      ...
      <ReaderTools mode={mode}>
        ...
        <ToolLink href={`${legacyBase}?tab=ethics`} label="Explore ethics and policy ↗" />
      </ReaderTools>
    </StorySection>
  );
}
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[.9fr_1.1fr]">
        ...
        <div>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            <DeepLink href={`${legacyBase}?tab=methods`} title="Methods & provenance" />
            <DeepLink href={`${legacyBase}?tab=library`} title="Disease catalogue" />
            <DeepLink href={`${legacyBase}?tab=editing-tech`} title="Editing technology gates" />
            <DeepLink href={`${legacyBase}?tab=funding`} title="Impact-funding analyses" />
          </div>
        </div>
      </div>
...
function ToolLink({ href, label }: { href: string; label: string }) {
  return (
    <div className="border-l border-slate-300 pl-4 text-xs leading-5">
      <p className="font-semibold text-slate-900">Explore the data</p>
      <a href={href} className="mt-1 inline-block text-blue-700 hover:text-blue-900">
        {label}
      </a>
    </div>
  );
...
function DeepLink({ href, title }: { href: string; title: string }) {
  return (
    <a href={href} className="border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 hover:border-blue-300 hover:text-blue-700">
      {title} ↗
    </a>
  );
}
...
function legacyRoot(): string {
  ...
  if (/\/v2\/?$/.test(path)) return path.replace(/\/v2\/?$/, '/');
  return '/';
}
