...
| B2 | **Variant-class assignments** for the editing gate ladder — 3 of 8 are **medium confidence**: beta-thalassaemia, Tay-Sachs, GJB2 deafness. *Every curated class is now confirmed attested in ClinVar for its gene (`make ingest`, enforced by `test_every_curated_dominant_class_is_attested`). What remains unconfirmed is which class **predominates among patients** — see C8.* | 8 conditions | ◐ |
| B3 | **Beta-thalassaemia is load-bearing** — it is the *entire* base-editable bucket in gate 2. *Evidence gathered: of the eight commonest HBB alleles, **five are transitions** (IVS-I-110, CD39, IVS-I-6, HbE, IVS-II-654) but **two are transversions** (IVS-II-745, CD17) and **one is a four-base deletion** (CD41/42) — all three of those need prime editing, not base editing. The 11% base-editable figure is therefore flagged an **upper bound** in the pipeline and the app. What is still missing is the weighting: the spectrum is strongly population-specific, and ~20 alleles cover >80% of cases out of >350 described. CFTR2 was checked as a model source and **cannot be used** — its terms forbid redistribution or derivative works, and this repo publishes its curated data. Needs a population-weighted allele-frequency table from the open literature.* | 1 condition | ◐ |
...
| C8 | **Per-condition allele spectra — allele *frequency*, not allele *diversity*.** ClinVar is ingested and settles which classes exist per gene, but **cannot** supply the weighting: it lists each allele once, so record counts put the SMN1 exon-7 deletion at 13% when ~95% of SMA patients carry it, and the HTT expansion at 15% when it causes essentially all Huntington's disease. Both errors inflate the correctable share. Needs patient-level sources: **CFTR2** (publishes allele frequencies among CF patients), **HbVar** for HBB, founder-allele series for HEXA and GJB2 | `ingest/clinvar.py` done; weighting open | ◐ |
